...
function openScheduleModal(index) {
  ...
  var students = JSON.parse(localStorage.getItem("students")) || [];
  var student = students[index];
  tempSchedule = student.schedule ? JSON.parse(JSON.stringify(student.schedule)) : [];
  renderScheduleTable();
...
function renderScheduleTable() {
  ...
  tempSchedule.forEach((sched, idx) => {
    ...
    row.insertCell(3).innerHTML = `<button type="button" onclick="removeScheduleRow(${idx})" style="background:#e74c3c;">Remove</button>`;
  });
}

function addScheduleRow() {
  var day = document.getElementById("schedDay").value.trim();
  var time = document.getElementById("schedTime").value.trim();
  var subject = document.getElementById("schedSubject").value.trim();
  if (day && time && subject) {
